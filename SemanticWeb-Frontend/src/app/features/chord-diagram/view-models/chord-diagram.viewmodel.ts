    ...
    public constructor(inputMatrix:number[][], radius: number, groupDetails : IDatabase[]){
        ...
        if(chords){
            chords.groups.forEach((group, index)=>{
                this.databaseVms.push(new DatabaseViewModel(group, radius, groupDetails[index].name, groupDetails[index].colour));
            });
            this.chordVms = chords.map(chord=>new ChordViewModel(chord, radius, this.databaseVms[chord.source.index], this.databaseVms[chord.target.index]));
        }
    }
}